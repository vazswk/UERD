...
for xxx = 1:length(CAPA)
    ...
    for w=1:length(files)        
        ...
        if files(w).isdir==0
            ...
            distortion =  nz_dct_v - nz_dct_coef; % rounding error   nz_dct_coef：舍入；nz_dct_v： 未舍入
...
%             decide = q_matrix./J; % version 1
            ...
            adjust_m = 1+2*(distortion <0).* distortion ; 
            adjust_p = 1-2*(distortion >0).* distortion ; 
            decide_m =decide.*(quant_rand.*adjust_m); 
            decide_p =decide.*(quant_rand.*adjust_p);
                      
            costs = zeros(3, length(nz_index), 'single'); % for each pixel, assign cost of being changed
            costs(1,:) = decide_m;       % cost of changing the first cover pixel by -1
            costs(3,:) = decide_p;       % cost of changing the first cover pixel by +1
